...
<div
  ...
  style="aspect-ratio: {SAFARI_WIDTH}/{SAFARI_HEIGHT}; {style}"
>
  {#if hasVideo}
    <div
    ...
    >
      <video
        class={cn("size-full object-coverblock size-full object-cover")}
        src={videoSrc}
        autoplay
        loop
        muted
  ...
  {#if !hasVideo && imageSrc}
    <div
      class={cn("pointer-events-none absolute z-0 overflow-hidden")}
      style="left: {LEFT_PCT}%; top: {TOP_PCT}%; width: {WIDTH_PCT}%; height: {HEIGHT_PCT}%; border-radius: 0 0 11px 11px;"
    >
      <img
        src={imageSrc}
        alt=""
  ...
  <svg
    ...
    class={cn("absolute inset-0 z-10 size-full pointer-events-none")}
  ...
  >
    ...
    >
      ...
      <path
        ...
        class={cn("fill-[#E5E5E5] dark:fill-[#404040]")}
      />
      <g class={cn("mix-blend-luminosity")}>
        <path
