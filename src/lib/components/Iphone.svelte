...
<div
  ...
  style="aspect-ratio: {PHONE_WIDTH}/{PHONE_HEIGHT}; {style}"
>
  {#if hasVideo}
    <div
      class={cn("pointer-events-none absolute z-0 overflow-hidden")}
      style="left: {LEFT_PCT}%; top: {TOP_PCT}%; width: {WIDTH_PCT}%; height: {HEIGHT_PCT}%; border-radius: {RADIUS_H}% / {RADIUS_V}%;"
    >
      <video
        class={cn("block size-full object-cover")}
        src={videoSrc}
        autoplay
        loop
        muted
  ...
  {#if !hasVideo && src}
    ...
    >
      <img
        {src}
        alt=""
        class={cn("block size-full object-cover object-top")}
      />
    </div>
  {/if}
  ...
  {#if children}
    <div
      class={cn("pointer-events-auto absolute z-0 overflow-hidden")}
      style="left: {LEFT_PCT}%; top: {TOP_PCT}%; width: {WIDTH_PCT}%; height: {HEIGHT_PCT}%; border-radius: {RADIUS_H}% / {RADIUS_V}%;"
    >
      {@render children()}
    </div>
  ...
  <svg
    ...
    class={cn("absolute inset-0 size-full pointer-events-none")}
    style="transform: translateZ(0);"
  >
    <g mask={hasMedia ? "url(#screenPunch-iphone)" : undefined}>
      <path
    ...
    <path
      ...
      mask={hasMedia ? "url(#screenPunch-iphone)" : undefined}
    />

    <path
      d="M154 48.5C154 38.2827 162.283 30 172.5 30H259.5C269.717 30 278 38.2827 278 48.5C278 58.7173 269.717 67 259.5 67H172.5C162.283 67 154 58.7173 154 48.5Z"
      class={cn("fill-[#F5F5F5] dark:fill-[#262626]")}
    />
    <path
    ...
    <defs>
      <mask id="screenPunch-iphone" maskUnits="userSpaceOnUse">
        <rect
          x="0"
          y="0"
          width={PHONE_WIDTH}
      ...
      <clipPath id="roundedCorners-iphone">
        <rect
          x={SCREEN_X}
          y={SCREEN_Y}
          width={SCREEN_WIDTH}
