<template>
  ...
  >
    ...
    <svg
      ...
      class="absolute inset-0 z-10 size-full pointer-events-none"
      style="transform: translateZ(0)"
    >
      <defs>
        <mask :id="`safariPunch-${id}`" maskUnits="userSpaceOnUse">
          <rect
            x="0"
            y="0"
            :width="SAFARI_WIDTH"
        ...
        <clipPath :id="`path0-${id}`">
          <rect :width="SAFARI_WIDTH" :height="SAFARI_HEIGHT" fill="white" />
        </clipPath>
      </defs>

      <g
        :clip-path="`url(#path0-${id})`"
        :mask="hasMedia ? `url(#safariPunch-${id})` : undefined"
      >
        <path
...
const props = withDefaults(defineProps<SafariProps>(), {
  mode: 'default',
  className: '',
  style: () => ({})
})
...
const hasMedia = computed(() => hasVideo.value || !!props.imageSrc || true) // children check implicitly true if slot used, simplified.
</script>
