<template>
  ...
  >
    ...
    >
      <defs>
        <mask :id="`screenPunch-${id}`" maskUnits="userSpaceOnUse">
          <rect
            x="0"
            y="0"
            :width="PHONE_WIDTH"
      ...
      <g :mask="hasMedia ? `url(#screenPunch-${id})` : undefined">
        <path
      ...
      <path
        ...
        :mask="hasMedia ? `url(#screenPunch-${id})` : undefined"
      />
...
const props = withDefaults(defineProps<IphoneProps>(), {
  className: '',
  style: () => ({})
})
...
const hasMedia = computed(() => hasVideo.value || !!props.src || true) // Simplified slot check
</script>
